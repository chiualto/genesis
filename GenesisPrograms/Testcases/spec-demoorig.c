begin genesis
	...
	generate 5 with dist1 = {1:10},numvardist = {2:32{99.5}},epochdist = {1:5},compdist = {1:5},dist2 = {1:10}
end genesis



__kernel
void kernel_mc02_orig(unsigned int outer_tc, unsigned int inner_tc, __global __volatile float *arr, int *result, int *result2){

	${varlistDeclare(int, temp)}
	${varlistDeclare(float, foo)}
	...
	${keepLive(result, temp)}
	${keepLive(result2, foo)}
