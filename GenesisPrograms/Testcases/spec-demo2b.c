begin genesis
	...
	feature computation 
		...
		add source1 to canbedefined
		add source2 to canbedefined
		add source3 to canbedefined
	...
	feature writevariables
	variable final from temp
		${final}
		remove final from temp
	end

	feature singleline(1) writaccess
		arr_out[i * n + j] = 
		genloop ${temp}>0
			${writevariables} 
			genif ${temp}!=0
	...
	feature loopinterchange
		value lifactor sample lidist report(1)
		genif ${lifactor}==0
